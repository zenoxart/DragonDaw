...
public sealed class ArrangementViewModel : INotifyPropertyChanged
{
    ...
    /// <summary>Snaps a beat to the grid. Holding Alt bypasses snap (FL-style).</summary>
    public double SnapToBeat(double beat)
        => SnapResolution > 0 && !IsSnapBypassed ? Math.Round(beat / SnapResolution) * SnapResolution : beat;

    private static bool IsSnapBypassed
    ...
    public string SnapDisplay  => _snapResolution switch { 0.0 => "OFF", 4.0 => "Bar", 2.0 => "1/2", 1.0 => "1/4", 0.5 => "1/8", 0.25 => "1/16", 0.125 => "1/32", _ => $"{_snapResolution:F2}" };
    ...
    {
        get => _snapResolution switch { 0.0 => 0, 4.0 => 1, 2.0 => 2, 1.0 => 3, 0.5 => 4, 0.25 => 5, 0.125 => 6, _ => 3 };
        set { SnapResolution = value switch { 0 => 0.0, 1 => 4.0, 2 => 2.0, 3 => 1.0, 4 => 0.5, 5 => 0.25, 6 => 0.125, _ => 1.0 }; OnPropertyChanged(nameof(SnapSelectedIndex)); }
    }

    /// <summary>
    /// Duplicates the selected clip directly behind itself on the same track
    /// (FL Studio: Strg+B). The copy becomes the new selection so repeated
    /// presses tile the clip across the timeline.
    /// </summary>
    public void DuplicateSelectedClip()
    {
        var vm = SelectedClip;
        if (vm == null) return;
        var track = Tracks.FirstOrDefault(t => t.Clips.Contains(vm));
        ...
        var copy = new ArrangementClip
        {
            ...
            StartBeat      = src.StartBeat + src.LengthInBeats,
            LengthInBeats  = src.LengthInBeats,
            Color          = src.Color,
        ...
        var copyVm = new ArrangementClipViewModel(copy, this);
        track.Clips.Add(copyVm);

        if (SelectedClip != null) SelectedClip.Model.IsSelected = false;
        SelectedClip = copyVm;
    }
    ...
    {
        // Tracks.Clear() raises a Reset action with NO OldItems — without this
        // branch the arrangement lanes (and all their clips!) would survive a
        // clear, leaving the playlist populated after "Datei → Neues Projekt".
