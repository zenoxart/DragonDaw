...
public sealed class PatternSequencerProvider : ISampleProvider
{
    ...
    public int Read(float[] buffer, int offset, int count)
    {
        ...
        RenderVoices(buffer, offset, frames, channels);
    ...
    /// <summary>
    /// Additively mixes all active voices into the block, sample-accurate.
    /// Voices play at their individual rate (Piano-Roll pitch) with linear
    /// interpolation per frame.
    /// </summary>
    private void RenderVoices(float[] buffer, int offset, int frames, int channels)
    {
